...
## 更新摘要
**变更内容**   
- 增强了Bevy图形适配器的材质系统，新增了fabric_material.rs自定义材质实现和fabric_material.wgsl WGSL着色器代码
- 实现了与材质系统的深度集成，支持更丰富的材质效果和渲染特性
- 更新了渲染管线以支持新的材质类型和着色器功能
...
**更新** 本次更新重点增强了材质系统，新增了fabric_material.rs自定义材质实现和fabric_material.wgsl WGSL着色器代码，实现了与材质系统的深度集成，支持更丰富的材质效果和渲染特性。
...
K["domain/material<br/>材质系统"]
L["fabric_material.rs<br/>自定义材质实现"]
M["fabric_material.wgsl<br/>WGSL着色器"]
end
D --> A
A --> F
A --> G
...
## 核心组件
- 渲染适配器模块：负责在 Bevy 世界中创建/更新实体、同步相机状态、驱动场景与瓦片集的帧级更新，并将领域对象转换为渲染所需的数据。**新增** 材质系统集成，支持自定义材质和着色器效果。
...
## 架构总览
下图展示从应用入口到渲染输出的关键调用链与数据流向，**更新** 包含新增的材质系统和Fabric材质支持。
...
participant FabricMat as "Fabric材质"
...
- 关键流程
  - 初始化：解析配置、建立与领域层的连接、准备资源缓存、**初始化材质系统**。
  - 每帧：读取输入→更新相机→更新场景→调度瓦片集更新→**更新材质状态**→提交渲染。
- 错误处理
  - 对资源加载失败、网络异常、无效几何等进行降级与日志记录。
  - 在不可恢复错误时回退到安全状态，避免崩溃。
  - **新增** 材质渲染错误处理：处理材质编译失败、着色器错误和材质属性验证。

**更新** 渲染适配器现在包含完整的材质系统集成，通过新的材质管理接口来支持Fabric材质和其他自定义材质类型。
...
## 结论
Bevy 渲染适配器通过清晰的职责划分与稳定的接口契约，将 Cesium 的领域能力无缝接入 Bevy 生态。**新增** 的材质系统增强进一步扩展了渲染能力，通过Fabric材质实现和WGSL着色器支持，提供了高质量的材质渲染和丰富的视觉效果。借助增量更新、批处理、并行化策略以及材质优化技术，可在大规模地理数据场景中实现高质量与高性能的渲染体验。建议在工程中结合调试与性能分析工具，持续优化瓦片调度、材质渲染和整体渲染路径。
...
- 材质开发指南
  - 学习Fabric材质格式规范和材质属性定义。
  - 掌握WGSL着色器编写和优化技巧。
  - 了解材质系统与Bevy渲染管线的集成方式。
