...
## 更新摘要
**所做更改**   
- 更新了基础层分辨率策略，BASE_LAYER_ZOOM以下的瓦片现在使用全分辨率获取而非降级版本
- 调整了瓦片下载优先级阈值从160.0到64.0以避免中间分辨率导致的视觉伪影
- 增加了内存使用量（从约5MB增加到约30MB）但确保了缩放过渡期间的清晰度
- 优化了瓦片加载质量与性能的平衡机制
...
**更新** 根据最新的代码变更，基础层分辨率策略已更新为在BASE_LAYER_ZOOM以下使用全分辨率瓦片，这显著提升了缩放过渡期间的视觉质量，同时通过调整优先级阈值避免了中间分辨率导致的视觉伪影。
...
N["dynamic_globe.rs<br/>隐藏瓦片LRU跟踪"] --> O["hide_order队列"]
O --> P["优化清理逻辑"]
```
...
- [dynamic_globe.rs:143-174](file://cesiumrust/application/cesium-app/src/dynamic_globe.rs#L143-L174)
...
- **新增** 隐藏瓦片LRU跟踪：通过hide_order队列记录离开渲染分区的瓦片顺序，实现高效的LRU淘汰策略。
- **新增** 优化清理逻辑：简化process_pipeline中的清理过程，专注于硬容量压力下的驱逐策略。
- 屏幕足迹优先级调度：根据瓦片在屏幕上的投影面积和距离视图中心的远近计算优先级，确保关键区域瓦片优先加载。
- 每帧预算管理机制：通过JobScheduler限制每帧的昂贵操作数量，将负载分散到多帧中，防止交互卡顿。
...
**更新** Rust 端瓦片处理组件现在提供更高效的底层实现，支持更好的并发处理和内存管理，同时集成了新的隐藏瓦片LRU跟踪机制和优化清理逻辑。
...
## 架构总览
下图展示从页面到渲染的关键流程，突出瓦片加载策略在其中的位置与作用点，包括新增的隐藏瓦片LRU跟踪和优化清理机制。
...
- [JobScheduler.js:53-85](file://packages/engine/Source/Scene/JobScheduler.js#L53-L85)
- [cache.rs:1-446](file://cesiumrust/domain/quadtree/src/cache.rs#L1-L446)
- [dynamic_globe.rs:422-486](file://cesiumrust/application/cesium-app/src/dynamic_globe.rs#L422-L486)
...
- [dynamic_globe.rs:452-453](file://cesiumrust/application/cesium-app/src/dynamic_globe.rs#L452-L453)

### 隐藏瓦片LRU跟踪机制
- **新增** hide_order队列：使用VecDeque<TileKey>维护离开渲染分区的瓦片顺序，实现先进先出的LRU策略。
- **新增** 自动跟踪：当瓦片不再可见时，自动添加到hide_order队列末尾，记录其离开时间。
- **新增** 智能驱逐：在内存压力下，优先驱逐hide_order队列中最旧的瓦片，确保系统稳定性。
- **新增** 清理优化：简化process_pipeline中的清理逻辑，专注于硬容量压力下的驱逐，提高处理效率。

```mermaid
flowchart TD
TileLeave["瓦片离开渲染分区"] --> AddQueue["添加到hide_order队列末尾"]
AddQueue --> CheckCapacity{"内存容量检查"}
CheckCapacity --> |正常| Continue["继续处理"]
CheckCapacity --> |超限| EvictOldest["驱逐最旧瓦片"]
EvictOldest --> RemoveFromCache["从缓存中移除"]
RemoveFromCache --> UpdateMetrics["更新统计指标"]
...
- [dynamic_globe.rs:143-174](file://cesiumrust/application/cesium-app/src/dynamic_globe.rs#L143-L174)
- [dynamic_globe.rs:452-453](file://cesiumrust/application/cesium-app/src/dynamic_globe.rs#L452-L453)
- [dynamic_globe.rs:853-872](file://cesiumrust/application/cesium-app/src/dynamic_globe.rs#L853-L872)
...
- **新增** 隐藏瓦片优化：利用hide_order队列实现更精确的LRU淘汰，提升快速缩放时的性能。
...
**更新** Rust 端组件提供了更精细的性能控制选项，支持动态调整缓存策略和并发限制，同时集成了新的隐藏瓦片LRU跟踪机制和优化清理逻辑。
...
- 模块耦合
  ...
  - **新增** dynamic_globe.rs集成隐藏瓦片LRU跟踪，与瓦片调度器紧密协作。
- 外部依赖
  - 网络请求与文件系统访问受平台能力限制，需考虑降级与超时策略。
- 潜在环路
  - 依赖图中若出现自引用或多跳回指，应通过构建期检测与运行时保护规避。

**更新** 移除了对已删除的 tiling_spec.rs 文件的依赖引用，简化了测试依赖关系，同时增加了新的隐藏瓦片LRU跟踪相关依赖。
...
- **新增** 隐藏瓦片优化：通过hide_order队列实现高效的LRU淘汰，减少快速缩放时的网络请求和渲染开销。

**更新** Rust 端组件的引入显著提升了瓦片处理的性能和内存安全性，减少了垃圾回收带来的停顿，同时新的隐藏瓦片LRU跟踪机制进一步优化了用户体验，特别是在快速缩放操作时表现更佳。
...
- 常见问题
  ...
  - **新增** 隐藏瓦片队列异常：检查hide_order队列的长度和清理逻辑，确保LRU淘汰正常工作。
- 定位步骤
  ...
  - **新增** 监控dynamic_globe.rs中的隐藏瓦片跟踪状态，验证LRU淘汰效果。

**更新** 新增了对 Rust 端问题的排查指导，包括内存安全和并发问题的诊断方法，同时增加了新隐藏瓦片LRU跟踪机制的故障排查指南。
...
**更新** 新增的隐藏瓦片LRU跟踪机制和优化清理逻辑显著提升了瓦片管理的效率和用户体验，特别是在快速缩放操作时减少了网络请求和渲染开销。基于屏幕足迹优先级的请求调度系统和每帧预算管理机制确保了关键区域瓦片优先加载的同时避免了交互卡顿。Rust 端组件的引入为瓦片加载策略提供了更强的性能和安全性保障，为未来的扩展奠定了坚实基础。
...
- 术语表
  ...
  - **新增** 隐藏瓦片LRU：通过队列跟踪离开渲染分区的瓦片，实现高效淘汰策略。
- 参考入口
  ...
  - **新增** 隐藏瓦片LRU跟踪实现位于 cesiumrust/application/cesium-app/src/dynamic_globe.rs。

**更新** 移除了对已删除的 tiling_spec.rs 文件的引用，更新了参考入口以反映当前的项目结构，并添加了新隐藏瓦片LRU跟踪机制的参考位置。
