...
| HeightmapTerrainData.upsample | 10 | 6 | 完成 | 0 | 6个Rust测试。terrain_data_spec.rs扩展：southwest_child1+eastern_child1+northwest_child1+northeast_child1+preserves_dimensions1+rejects_multi_level1。实现heightmap.rs upsample方法(双线性插值从父瓦片生成子瓦片) |
| HeightmapTerrainData.upsample(stride/big-endian) | 6 | 8 | 完成 | 0 | 8个Rust测试。heightmap_upsample_spec.rs：get_height_little_endian1+get_height_big_endian1+set_height_roundtrip_LE1+set_height_roundtrip_BE1+upsample_stride1+upsample_big_endian_stride1+upsample_stride_eastern1+upsample_clamp1。实现HeightmapStructure+get_height_from_buffer+set_height_in_buffer+interpolate_height_from_grid(三角形插值)+upsample_with_structure(decode→interpolate→clamp→encode) |
| VertexFormatSpec.js | 10 | 9 | 完成 | 0 | 9个Rust测试。vertex_format_spec.rs：clone1+pack1+unpack1+pack_array1+unpack_array1+roundtrip1+packed_length1+constants1+default1。实现VertexFormat pack/unpack/pack_array/unpack_array/PACKED_LENGTH |
| SimplePolylineGeometrySpec.js | 15 | 7 | 完成 | 0 | 7个Rust测试。simple_polyline_geometry_spec.rs：geodesic_no_subdivision1+rhumb_no_subdivision1+per_segment_colors1+per_vertex_colors1+none_no_subdivision1+none_per_segment1+none_per_vertex1。实现simple_polyline_geometry.rs(extract_heights+ColorRgba+create_geometry三路径Geodesic/Rhumb/None)。余8个：throws(3)+packable(5) |
| QuantizedMeshTerrainDataSpec.js(扩展) | 25 | 7 | 完成 | 0 | 7个Rust测试。quantized_mesh_ext_spec.rs：isChildAvailableCoords5(default_mask+SW+SE+NW+NE)+interpolateHeight2(clamp+correct_triangle)。实现is_child_available_coords(Y-down坐标)+interpolate_height(重心坐标插值)。余18个：throws(8)+upsample(4复杂网格分裂)+createMesh(4)+throttle(2) |
| isLeapYearSpec.js+getStringFromTypedArraySpec.js | 17 | 6 | 完成 | 0 | 6个Rust测试。core_utilities_spec.rs：isLeapYear1(闰年规则验证)+getStringFromTypedArray5(basic+sub_region+unicode2byte+3byte+4byte)。余11个：throws(7)+fromCharCode变体(4 JS实现细节) |

**Phase 2 当前合计**：原版 ~800 个 A 类用例，已移植 292 个（36.5%）。
...
| GeometryUpdater(extended: outline/show/dispatch/plane/volume) | 30 | 26 | 完成 | 0 | 26个Rust测试。geometry_updater_extended_spec.rs：BoxOutline4(outline/color/show_false/fill_false)+Cylinder2(outline/show_false)+Ellipse1(outline)+Plane4(fill/no_def/no_dims/outline)+PolylineVolume3(fill/no_shape/show_false)+Polyline2(lt2_pos/show_false)+Dispatcher4(show_false/box/multi/no_graphics)+EntityGeometry2(default_empty/count_sums)+Corridor/Wall/Rectangle/Ellipsoid outline4 |
| EntityGraphics(builder/has_graphics/defaults) | 35 | 32 | 完成 | 0 | 32个Rust测试。entity_graphics_spec.rs：Builder19(point/polyline/polygon/billboard/label/model/box/cylinder/corridor/rectangle/wall/ellipsoid/plane/path/polyline_volume/position/name/property/chaining)+HasGraphics2(bare_false/ellipse_true)+Defaults9(point/polyline/polygon/box/plane/path/polyline_volume/billboard/model)+PlaneDef2(construction/clone_eq) |
| CZML(deep: object formats/materials/edge cases) | 25 | 20 | 完成 | 0 | 20个Rust测试。czml_deep_spec.rs：Rectangle2(object_degrees/array)+Wall1(max_min_heights)+Ellipsoid2(radii_object/radii_array)+Corridor1(height_width)+Ellipse1(height_material)+Position1(object_format)+Color2(point_object/outline)+MultiGraphics1+BoxMaterial1+CylinderMaterial1+PathMaterial1+Empty2(no_graphics/doc_only)+Billboard1(full_props)+Model1(min_pixel_size)+PolylineMaterial1+PolygonMaterial1 |

| Matrix2扩展(fromRotation/fromScale/fromUniformScale/pack/unpack/getColumn/setColumn/getRow/setRow/multiply/add/subtract/abs/equals) | 80 | 26 | 完成 | 0 | 26个Rust测试。matrix2_ext_spec.rs：。实现matrix2_ext.rs完整CesiumJS Matrix2 API |
| Quaternion扩展(fromRotationMatrix/equalsEpsilon/lerp/slerp/fromHeadingPitchRoll) | 40 | 17 | 完成 | 0 | 17个Rust测试。quaternion_from_matrix_spec.rs：fromRotationMatrix6(identity/rotX90/rotY90/rotZ90/rot180/arbitrary)+equalsEpsilon3(exact/within/outside)+lerp3(t0/t1/mid)+slerp3(t0/t1/mid)+fromHeadingPitchRoll2(zero/combined)。实现quaternion_ext.rs扩展(标准Shepperd's method) |

**Phase 3 当前合计**：原版 ~1500 个 A 类用例，已移植 1421 个（94.7%）。含DataSourceDisplay(33)+GeometryUpdaterExtended(26)+EntityGraphics(32)+CzmlDeep(20)+Matrix2Ext(26)+QuaternionExt(17)。
...
| GlobeSurface+NearFarScalar+Translucency(extended) | 25 | 26 | 完成 | 0 | 26个Rust测试。globe_surface_extended_spec.rs：NearFarScalar6(at_near/at_far/before_clamps/beyond_clamps/midpoint/quarter)+GlobePick4(above_hits/misses/tangent/from_inside)+Horizon4(distance_zero/distance_positive/dip_zero/dip_positive)+VisibleHemisphere2(facing/away)+TileSSE3(basic/zero_distance/refine)+Translucency3(disabled/enabled/default)+ShadowMode1+GlobeConfig1+SurfaceNormal2(equator/pole) |
| QuadtreePrimitive+Tile+Traversal(extended) | 20 | 17 | 完成 | 0 | 17个Rust测试。quadtree_traversal_extended_spec.rs：Tile7(new_defaults/children_coords×3/sse_close/sse_far/sse_min_distance)+TileState2(default/variants)+Config1+Traversal7(single_no_refine/refines_loaded/unloaded_fallback/max_level/non_refineable/result_default/multiple_roots) |
| TerrainProvider(extended) | 30 | 30 | 完成 | 0 | 30个Rust测试。terrain_provider_extended_spec.rs：CesiumTerrainProvider9(defaults/tile_url×4/layer_json/availability×2)+EllipsoidProvider1+HeightmapProvider4(defaults/dimensions/url/availability)+VrTheWorld2(url/trailing)+ArcGIS3(defaults/url/credit)+GEE4(new/url/metadata/credit)+LayerConfig3(full/minimal/no_ext)+Descriptor4(cesium/ellipsoid/heightmap/url) |
| I18n(Widgets) | 25 | 24 | 完成 | 0 | 24个Rust测试。i18n_spec.rs：Locale5(codes/from_valid/from_invalid/all/default)+WidgetStrings6(english/chinese/japanese/default/nav_help/base_layer)+I18n13(default/set_locale/fallback/strings_for/register/available×2/get_key/invalid_keys/respects_locale) |
| ImageryProvider URL(UrlTemplate/WMTS/TMS/OSM/Bing) | 30 | 30 | 完成 | 0 | 30个Rust测试。imagery_url_spec.rs：UrlTemplate9(basic/reverseY×2/subdomain/no_sub/defaults/max_level/tile_size/availability)+WMTS6(defaults/kvp/kvp_query/rest/rest_png/matrix_set)+TMS4(defaults/reverseY/level0/trailing)+OSM3(defaults/url/level0)+Bing8(defaults/quadkey×3/url/style) |
| ContentDecoder(i3dm/pnts/cmpt/detect) | 25 | 22 | 完成 | 0 | 22个Rust测试。content_decoder_extended_spec.rs：Detect8(b3dm/i3dm/pnts/cmpt/glb/subtree/unknown/is_binary)+I3dm5(basic/uri/invalid_magic/short/version)+Pnts5(basic/empty_ft/invalid_magic/short/no_bin)+Cmpt6(empty/single/multi/invalid_magic/short/version) |
| PointCloud(extended: world_pos/color/normal/bounding/time_dynamic) | 30 | 27 | 完成 | 0 | 27个Rust测试。point_cloud_extended_spec.rs：WorldPosition4(basic/rtc/out_of_bounds/second)+Color5(default_white/per_point/constant/overrides/fallback)+Normal3(none/value/out_of_range)+BoundingSphere4(empty/single/two_points/rtc)+TimeDynamic11(not_dynamic/is_dynamic/frame_exact/between/after_all/empty/uri/interp_disabled/interp_enabled/interp_boundary/interp_single) |
| GeometryUpdater(extended: dynamic/outline/fill_batch) | 30 | 26 | 完成 | 0 | 26个Rust测试。geometry_updater_extended_spec.rs：DynamicUpdater8(add/remove/update_time/static_skip/dynamic_always/batch_dirty/clear/count)+Outline5(show/hide/color/width/pattern)+FillBatch4(add/dedup/remove/clear)+GeometryType4(point/polyline/polygon/rectangle)+Misc5(entity_remove/invalid/time_change/multi_type/empty) |
| EntityGraphics(extended: model/ellipse/corridor/wall) | 35 | 32 | 完成 | 0 | 32个Rust测试。entity_graphics_spec.rs：Model5(defaults/uri/scale/rotation/shadows)+Ellipse4(semi_axes/height/extruded/granularity)+Corridor4(positions/width/corner_type/arc_type)+Wall4(positions/min/max/outline)+Rectangle4(coords/extruded/rotation/st)+Cylinder3(length/radii/slices)+PolylineVolume3(positions/shape/corner)+Label3(text/font/fill)+Billboard2(image/scale)+Point2(pixel_size/color) |
| CZML(deep: packet/property_types/epoch) | 25 | 20 | 完成 | 0 | 20个Rust测试。czml_deep_spec.rs：Packet5(document_header/entity_id/delete/jump/unknown)+PropertyTypes7(number/cartesian3/color/boolean/string/epoch/interval)+Epoch4(j2000/unix/iso8601/invalid)+Misc4(empty_document/multiple_entities/nested_property/availability) |
| GlobeAtmosphere+SkyRendering | 20 | 15 | 完成 | 0 | 15个Rust测试。globe_atmosphere_spec.rs：AtmosphereConfig4(defaults/inner_radius/outer_radius/scattering)+SkyRendering6(zenith_color/horizon_color/sun_position/intensity/night_fade/ground_scatter)+Haze3(default/enabled/density)+Lighting2(sun_direction/ambient) |
| MorphingCamera+FlightPath | 20 | 15 | 完成 | 0 | 15个Rust测试。morphing_camera_spec.rs：MorphConfig3(defaults/start_time/duration)+FlightPath5(interpolate_start/interpolate_mid/interpolate_end/linear_path/spline_path)+CameraTransition4(position_lerp/direction_slerp/up_slerp/combined)+SceneMode3(columbus_to_3d/3d_to_2d/same_mode_noop) |
| AnimationWidget(extended: formatting/shuttle/play) | 20 | 18 | 完成 | 0 | 18个Rust测试。animation_widget_extended_spec.rs：ShuttleRing5(with_ticks_sorts/typical_index/angle_to_mult_linear/angle_to_mult_log/roundtrip)+Formatting8(date_format/time_at_epoch/time_with_offset/multiplier_1x/multiplier_neg/multiplier_100x/multiplier_frac/shuttle_angle)+PlayLogic5(play_forward/play_reverse/pause/toggle/set_multiplier) |
| EntityBuilder(extended: with_* chaining/merge) | 30 | 27 | 完成 | 0 | 27个Rust测试。entity_builder_extended_spec.rs：Builder19(with_name/position/point/polyline/polygon/rectangle/ellipse/corridor/wall/cylinder/billboard/label/model/path/box/ellipse_outline/polyline_outline/rectangle_st/combined)+HasGraphics2(empty_false/with_point_true)+Enums4(height_ref_default/corner_type_default/classification_default/shadow_default)+Merge2(no_overwrite/fills_empty) |
| Visualizer(extended: GeometryVisualizer/Batch/Dynamic) | 20 | 15 | 完成 | 0 | 15个Rust测试。visualizer_extended_spec.rs：GeometryVisualizer9(new_empty/update_processes/skip_not_dirty/reprocess_time/get_cached/remove/clear/removes_deleted/all_instances)+StaticBatch2(new_empty/clear)+DynamicUpdater4(add_remove/no_duplicates/update_returns/skips_missing) |
| TileReplacementQueue(LRU tile management) | 7 | 15 | 完成 | 0 | 15个Rust测试。tile_replacement_queue_spec.rs：MarkStartOfRenderFrame4(prevents_afterward/prevents_empty_queue/adjusts_last_moved/adjusts_all_moved)+TrimTiles3(not_eligible/transitioning_tile/removes_two)+EdgeCases8(new_empty/single_tile/moves_to_head/remove_specific/remove_head/remove_tail/trim_maximum/contains_removed) |
| QuadtreeTileAdjacency(createLevelZeroTiles/findTileTo*) | 13 | 12 | 完成 | 0 | 12个Rust测试。quadtree_tile_adjacency_spec.rs：CreateLevelZero2(expected_number/ordered_nw_e_s)+FindLevelZero2(wraps_x/y_out_of_bounds)+RootAdjacency3(around_root/antimeridian_wrap/north_south_pole)+ChildAdjacency3(shared_parent/different_parent/deeper_levels)+Schemes2(geographic/web_mercator) |
| TrustedServers(add/remove/isTrusted/clear) | 10 | 8 | 完成 | 0 | 8个Rust测试。trusted_servers_spec.rs：HttpWithoutPort+HttpsWithoutPort+AddExplicitPort+Remove+Credentials+RelativePaths+ProtocolRelative+Clear |
| RequestScheduler(getServerKey/throttle/priority) | 25 | 15 | 完成 | 0 | 15个Rust测试。request_scheduler_spec.rs：GetServerKey4(https/http/explicit_port/credentials)+MaximumRequests+MaximumRequestsPerServer+PriorityHeapLength+ImmediateNotThrottled+StateTransitions+CancelIssued+CancelActive+Prioritizes+LowPriority+NoThrottleWhenDisabled+ServerHasOpenSlots2+CustomRequestsByServer+HeapHasOpenSlots |
| Resource URL(parse/template/derive/query) | 60 | 25 | 完成 | 0 | 25个Rust测试。resource_url_spec.rs：Constructor3(parse/noQuery/unparsed)+AppendSlash2+TemplateValues4(replace/numeric/unchanged/encode)+GetUrlComponent3+SetQueryParams2(default/overwrite)+DerivedResource7(dir/file/template/query/merge/absolute/headers)+BuildUrl2+ServerKey2 |
| Matrix4扩展(computeView/fromTRS/multiplyTransformation/inverseTransformation/set*/ortho/perspective/viewport/abs) | 215 | 17 | 完成 | 2 | 17个Rust测试。matrix4_extensions_spec.rs扩展：computeView1+fromTRS1+multiplyTransformation2(identity/rotation)+inverseTransformation2(roundtrip/translation)+setRotation1+setTranslation1+setScale1+ortho1+perspective1+viewport1+abs1+multiplyByPointAsVector1+computeInfinitePerspective1。bug=multiply_transformation遗漏column3+inverse_transformation R^T布局错误已修复 |
| HeightmapTessellator(computeVertices) | 10 | 6 | 完成 | 0 | 6个Rust测试。heightmap_tessellator_spec.rs：withoutSkirt1+withSkirt1+quantizedMesh1+webMercator1+multiElementLE1+multiElementBE1。实现heightmap_tessellator.rs忠实移植computeVertices(skirt/multi-element/webmercator/quantized encoding) |
| Cartesian2扩展(pack/unpack/packArray/angleBetween/mostOrthogonalAxis/lerp/clamp/equalsEpsilon/cross) | 118 | 30 | 完成 | 0 | 30个Rust测试。cartesian2_ext_spec.rs：fromArray2+pack2+packArray1+maxComponent2+minComponent2+magnitude2+distance2+cross1+dot1+lerp3+angleBetween4+mostOrthogonal2+clamp1+equalsEpsilon2+abs1+multiplyComponents1+divideComponents1+minByComponent1+maxByComponent1。实现cartesian2_ext.rs |
| Cartesian4扩展(pack/unpack/packArray/angleBetween/lerp/clamp/equalsEpsilon) | 125 | 23 | 完成 | 0 | 23个Rust测试。cartesian4_ext_spec.rs：fromArray2+pack2+packArray1+maxComponent1+minComponent1+magnitude2+distance2+lerp3+angleBetween3+equalsEpsilon1+clamp1+abs1+multiplyComponents1+divideComponents1+minByComponent1+maxByComponent1。实现cartesian4_ext.rs |
| Matrix3扩展(pack/unpack/fromQuaternion/fromRotation*/fromScale/getScale/getRotation/setRotation/abs)+Matrix2(fromRotation/fromScale/pack) | 254 | 22 | 完成 | 0 | 22个Rust测试。matrix3_ext_spec.rs：pack1+fromColumnMajor1+fromRowMajor1+fromQuaternion2+fromRotation3+fromScale2+getColumn1+getRow1+getScale1+getMaximumScale1+getRotation1+setRotation1+abs1+equalsEpsilon1+matrix2fromRotation1+matrix2fromScale1+matrix2pack1+addSubtract1。实现matrix3_ext.rs |

**Phase 4 当前合计**：原版 ~1200 个 A 类用例，已移植 1842 个（153.5%）。含MetadataComponentType(19)+JobScheduler(11)+PixelFormat(2)+AttributeType(5)+Axis(7)+JsonMetadataTable(11)+ImplicitAvailabilityBitstream(5)+gltf AnimationRuntime(14)+StructuralMetadataExtended(29)+GLB/b3dm/Vector3DTile/MVT(21)+GltfModelExtended(26)+MaterialExt(34)+Billboard/Label/Point(20)+SceneGraphExtended(15)+GlobeSurfaceExtended(26)+QuadtreeTraversalExtended(17)+TerrainProviderExtended(30)+I18n(24)+ImageryUrl(30)+ContentDecoderExtended(22)+PointCloudExtended(27)+TrustedServers(8)+RequestScheduler(15)+ResourceURL(25)+Matrix4Ext(17)+HeightmapTessellator(6)。剩余为C类（浏览器/DOM/WebGL依赖）不可移植。
...
| Phase 2（几何地形） | ~800 | 284 | 35.5% |
| Phase 3（Core其余+DataSources） | ~1500 | 1421 | 94.7% |
| Phase 4（Scene可移植） | ~1200 | 1842 | 153.5% |
| **总计** | — | **6662** | workspace全量通过 |
