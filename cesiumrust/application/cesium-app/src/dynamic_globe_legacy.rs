...
// frozen legacy golden-path style debt; local allow to satisfy strict CI clippy gate; revisit at M1.5 薄壳化 (deferred.md #18)
...
// M1.5: shared base_sphere types (both thin-shell and legacy consume the same
// ECS component/resource types so main.rs BaseSpherePlugin works with either).
...
// M1.5: TilePipelineSet is canonically defined in the thin-shell module;
// legacy re-exports the same type so perf_trace `.after(TilePipelineSet)`
// works regardless of which plugin is active.
pub(crate) use crate::dynamic_globe::TilePipelineSet;
...
// M0.4 added `ResMut<PerfCounters>` as the 8th Bevy system parameter (the
// counter write-back surface). Bevy systems legitimately take one parameter
// per resource access, so the clippy 7-arg heuristic does not apply; the
// alternative (splitting the write-back into a second system) would force the
// per-frame delta locals — mesh_uploads / tex_uploads / spawns / removed — to
// be smuggled out through TileManager fields, which is MORE invasive than one
// extra observer parameter. Allow scoped to this function only.
...
fn process_pipeline(
    ...
    // M0.4: reset per-frame deltas before the pipeline runs. Gauges and
    // cumulative totals are written at the end of this function.
    perf.begin_frame();
    ...
    while mesh_uploads < MAX_MESH_UPLOADS_PER_FRAME {
        ...
        if is_fb {
            ...
            let (e, d) = evict_gpu_cache(&mut mgr);
            evict_total_delta += e;
            evict_deferred_delta += d;
        } else {
    ...
    while tex_uploads < MAX_TEXTURE_UPLOADS_PER_FRAME {
        ...
        if result.aborted {
            ...
            // M0.4: count the stale skip (pure observation, no logic change).
            stale_skips_delta += 1;
            continue;
        }
        if result.failed {
    ...
    // ── M0.4: PerfCounters write-back (pure observation, zero logic) ──────
    // Gauges: read straight from the live collections.
    ...
    // Per-frame deltas: the local counters the pipeline already maintained.
    ...
    // Cumulative totals: add this frame's contribution.
...
///
/// M0.4: returns `(evicted, deferred)` counts for PerfCounters. The eviction
/// **logic is unchanged** — this is pure observation.
fn evict_gpu_cache(mgr: &mut TileManager) -> (u32, u32) {
...
// M1.5: BaseSphereMarker, BaseSphereComposite, COMPOSITE_TILE, COMPOSITE_SIZE
// are imported from the shared `base_sphere` module (see top-of-file imports).
...
fn base_sphere_composite_system(
    ...
    if let Some(rx) = &state.rx {
        ...
        let handle =
            base_sphere::make_clamped_image(&mut images, chain, COMPOSITE_SIZE, COMPOSITE_SIZE, levels);
...
// M1.5: box_downsample, ocean_block, make_clamped_image are now in the shared
// `base_sphere` module (imported at top of file). Local copies removed.
