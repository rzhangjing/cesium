//! Ported from `packages/engine/Source/DataSources/RectangleGeometryUpdater.js`.

use crate::entity::Entity;
use crate::geometry_updater::GeometryUpdater;

/// A geometry updater that creates rectangle geometry instances from entity data.
pub struct RectangleGeometryUpdater {
    entity_id: String,
    fill_enabled: bool,
    outline_enabled: bool,
}

impl RectangleGeometryUpdater {
    /// Creates a new rectangle geometry updater.
    ...
    pub fn create_fill_geometry_instance(&self, _entity: &Entity) -> Option<Vec<u8>> {
        // DEVIATION: Requires full rectangle geometry construction
        None
    }

    /// Creates the geometry instance for outline.
    pub fn create_outline_geometry_instance(&self, _entity: &Entity) -> Option<Vec<u8>> {
        // DEVIATION: Requires full rectangle outline geometry construction
        None
    }
}

impl GeometryUpdater for RectangleGeometryUpdater {
