//! Ported from `packages/engine/Source/DataSources/WallGeometryUpdater.js`.

use crate::entity::Entity;
use crate::geometry_updater::GeometryUpdater;

/// A geometry updater that creates wall geometry instances from entity data.
pub struct WallGeometryUpdater {
    entity_id: String,
    fill_enabled: bool,
    outline_enabled: bool,
}

impl WallGeometryUpdater {
    /// Creates a new wall geometry updater.
    ...
    pub fn create_fill_geometry_instance(&self, _entity: &Entity) -> Option<Vec<u8>> {
        // DEVIATION: Requires full wall geometry construction
        None
    }

    /// Creates the geometry instance for outline.
    pub fn create_outline_geometry_instance(&self, _entity: &Entity) -> Option<Vec<u8>> {
        // DEVIATION: Requires full wall outline geometry construction
        None
    }
}

impl GeometryUpdater for WallGeometryUpdater {
