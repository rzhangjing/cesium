//! Ported from `packages/engine/Source/DataSources/CylinderGeometryUpdater.js`.

use crate::entity::Entity;
use crate::geometry_updater::GeometryUpdater;

/// A geometry updater that creates cylinder geometry instances from entity data.
pub struct CylinderGeometryUpdater {
    entity_id: String,
    fill_enabled: bool,
    outline_enabled: bool,
}

impl CylinderGeometryUpdater {
    /// Creates a new cylinder geometry updater.
    ...
    pub fn create_fill_geometry_instance(&self, _entity: &Entity) -> Option<Vec<u8>> {
        // DEVIATION: Requires full cylinder geometry construction
        None
    }

    /// Creates the geometry instance for outline.
    pub fn create_outline_geometry_instance(&self, _entity: &Entity) -> Option<Vec<u8>> {
        // DEVIATION: Requires full cylinder outline geometry construction
        None
    }
}

impl GeometryUpdater for CylinderGeometryUpdater {
