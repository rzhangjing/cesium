//! Ported from `packages/engine/Source/DataSources/EllipseGeometryUpdater.js`.

use crate::entity::Entity;
use crate::geometry_updater::GeometryUpdater;

/// A geometry updater that creates ellipse geometry instances from entity data.
...
impl EllipseGeometryUpdater {
    /// Creates a new ellipse geometry updater.
    ...
    pub fn create_fill_geometry_instance(&self, _entity: &Entity) -> Option<Vec<u8>> {
        // DEVIATION: Requires full ellipse geometry construction
        None
    }

    /// Creates the geometry instance for outline.
    pub fn create_outline_geometry_instance(&self, _entity: &Entity) -> Option<Vec<u8>> {
        // DEVIATION: Requires full ellipse outline geometry construction
        None
    }
}
