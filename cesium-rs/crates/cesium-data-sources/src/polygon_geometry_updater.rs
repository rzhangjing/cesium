//! Ported from `packages/engine/Source/DataSources/PolygonGeometryUpdater.js`.

use crate::entity::Entity;
use crate::geometry_updater::GeometryUpdater;

/// A geometry updater that creates polygon geometry instances from entity data.
pub struct PolygonGeometryUpdater {
    entity_id: String,
    fill_enabled: bool,
    outline_enabled: bool,
}

impl PolygonGeometryUpdater {
    /// Creates a new polygon geometry updater.
    ...
    pub fn create_fill_geometry_instance(&self, _entity: &Entity) -> Option<Vec<u8>> {
        // DEVIATION: Requires full polygon geometry construction
        None
    }

    /// Creates the geometry instance for outline.
    pub fn create_outline_geometry_instance(&self, _entity: &Entity) -> Option<Vec<u8>> {
        // DEVIATION: Requires full polygon outline geometry construction
        None
    }
}

impl GeometryUpdater for PolygonGeometryUpdater {
