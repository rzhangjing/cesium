...
//!
//! DEVIATION: CesiumJS keeps `SceneMode` under `Source/Scene`. The Rust port
//! places it in `cesium-core` because core-level types that mirror the JS
//! picking path (`TerrainPicker`, `TerrainMesh`) branch on it, and
//! `cesium-core` cannot depend on `cesium-scene`. `cesium_scene::scene_mode`
//! re-exports this type, so every pre-existing import path keeps working.
...
pub enum SceneMode {
    ...
    Morphing = 0,
    /// Columbus View mode - a 2.5D perspective where the map is flat.
    ColumbusView = 1,
    /// 2D mode - top-down orthographic projection.
    Scene2D = 2,
    /// 3D mode - traditional 3D perspective of the globe.
    Scene3D = 3,
}
